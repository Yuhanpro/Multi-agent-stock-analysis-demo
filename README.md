...
- **大盘指数条始终渲染并放大字号** —— overview 顶部大盘指数条从按需渲染改为常驻显示,并放大字号提升可读性。
- **板块下钻挂载 + 热点桑基移出 boot** —— 把热点桑基的行业图预热线程从启动 boot 阶段移出改为按需挂载,避免与 overview 启动预热并发轰新浪触发限流;板块下钻改为懒挂载。

阻塞项 / 遗留:

- 无。

### 2026-07-05 — 资金下钻移动端改为左右分栏;修复市场热度空白

交付内容:

- **资金下钻手机端左右布局** —— 之前手机上是"行业列表在上、个股在下"两个长列表纵向堆叠,难扫读。改为手机也左右分栏:左侧 104px 窄列做行业竖排 tab(名称 + 净额上下排,隐藏条形图),右侧个股明细(名称弹性宽度 + 涨跌幅 + 净额,隐藏条形与代码);≥640px 恢复原有带条形图的宽排版。两侧独立滚动(手机 max-h-96)。
- **修复"市场热度刷不出来"** —— 根因是新浪单 IP 并发限流的连锁反应:①启动时 4 个预热线程(基金/市场热度×3/热点桑基行业图/美股 ETF)同时轰新浪;②CN `_compute` 内部又把 70 页的 `stock_zh_a_spot` 大爬取和指数/行业小请求并行发;限流窗口里所有接口返回垃圾,**空结果被当有效数据缓存**,且每 5 分钟的后台刷新继续自己触发限流、用空数据续命。三处修复:启动预热改为单线程串行(overview→hotspot→global-flow→funds);CN 计算改两阶段(先并行 4 个轻请求,再单独跑 spot 大爬取);新增 last-good 保护(`_gutted`/`_store`),空结果永不覆盖已有好数据、只回拨时间戳提前重试,warm 时空结果等 45s 重试一次。修复后公网验证:指数 5 条、行业 12、公司 20、涨跌家数 1867/64 涨停,预热日志 `gutted=False`。

阻塞项 / 遗留:

- 无。

### 2026-07-04 — 热点雷达扩展:美股板块资金方向 + 港股南向资金

交付内容:

- **美股板块资金方向(近似)** —— 新增 `backend/app/services/global_flow.py`:用 11 只 SPDR 行业 ETF(XLK/XLF/XLE 等)+ SPY/QQQ 的「涨跌幅 + 美元成交额 + 量比」近似板块资金方向(涨且放量≈流入),数据走 VPS 可达且稳定的 `stock_us_daily`(EOD 日线)。启动预热(~13 次请求 ≈30s)+ 30 分钟 TTL + stale-while-revalidate,与 hotspot 桑基图同一缓存模式。页面明确标注"基于日线收盘的方向近似,非真实订单流"。
- **港股南向资金(真实)** —— 接入 `stock_hsgt_fund_flow_summary_em`,展示港股通(沪)/(深)成交净买额与南向合计(交易所公布,真实资金)。
- **新端点 `GET /api/global-flow`** —— 挂在 hotspot 路由下;前端热点雷达页在 A 股桑基图之后新增「美股板块资金方向」条形榜(含 SPY/QQQ 基准 chips、量比高亮)和「港股南向资金」大数字卡,加载为 best-effort,不阻塞 A 股区块。
- **公网验证** —— `/api/global-flow` 缓存命中 1.6s 返回:美股 7-02 科技(XLK)-2.71% 放量流出、金融/医疗领涨流入,与 QQQ -1.73% 一致;港股通 7-03 南向净买 沪 +44.7 亿、深 +0.7 亿。
- **热点雷达信息架构重组** —— 用户反馈内容重复、不知道重点看什么。重组为四个有阅读顺序的分区:①今日结论(涨停/炸板/连板与阶梯 chips 合并进结论卡,删掉重复的数字条)→ ②钱往哪流·真实资金(下钻面板为主视图 + 题材榜;删掉与下钻重复的同花顺行业榜;两张桑基图收进「展开桑基图」折叠)→ ③海外资金·参考(港股南向 + 美股近似合并为一行两列)→ ④涨停与打板结构·强度代理(涨停主线/封板榜/放量榜/量能加速归组,并显式标注"非真实资金流")。每个分区有标题 + 一句"看什么"。
- **资金流向点击下钻(行业 → 个股)** —— 桑基图那份全 A 个股级真实净流入(同花顺·即时)原来只裁剪成 top7×3 画图;现在完整暴露为 `GET /api/flow-drill`(与桑基共享同一 3 分钟缓存,49 个行业,每行业保留最强流入 14 只 + 最强流出 10 只,含最新价/涨跌幅)。前端热点雷达新增「资金下钻 · 点击行业看个股」面板:左侧行业净额条形列表可点击,右侧显示该行业个股按净流入排序。修复 THS `涨跌幅` 返回 `"20.02%"` 字符串导致解析为空的问题。公网验证:点击「机械行业 +37.1亿」下钻到中际旭创 +25.52亿(-2.36%)、三一重工 +2.86亿(+4.45%)等。

阻塞项 / 遗留:

- 美股为 EOD 近似,盘中方向不可用;真实美股订单流需付费数据源。
- 东财系美股 spot(`stock_us_famous_spot_em` 等)在本 VPS 仍不可达,故未用于盘中刷新。

### 2026-07-03 — 热点雷达真实资金流桑基图(双向)

交付内容:

- **接入真实资金流向** —— `hotspot` 接同花顺行业 / 概念即时净流入,用真实资金而非涨幅近似。
- **资金下钻桑基图(打板抢筹 → 行业 → 个股)** —— 净流入 → 行业 → 个股三段桑基,呈现资金抢筹路径。
- **真实资金流入 / 流出双向桑基** —— 净流入 → 行业 → 个股,流入 / 流出双向分支,直观对比主力方向。

阻塞项 / 遗留:

- 真实资金为同花顺即时口径,刷新频率受限于数据源。
...
### 2026-07-02 — 热点雷达地基 + market-heat 重构 + admin 口径修正 + 移动端

交付内容:

- **热点雷达(A 股)地基** —— 用 VPS 可达数据近似资金抢筹方向;顶部新增结论卡(情绪判定 + 今日热点行业);AI 热点复盘 + 量能加速榜 + 主线持续性;`refactor` 与市场热度去重并明确分工(热点雷达管资金抢筹,市场热度管成交活跃)。
- **overview 跳转** —— 「→热点雷达」从文字链接改为醒目按钮条(accent 底 + 图标 + 箭头);两页跳转按钮加「点进去能看到什么」说明(标题 + 一句话)。
- **admin 口径修正** —— 访客等日粒度统计改用北京时间(UTC+8);新访客 / 回访改为按周期正确统计;累计型指标(注册用户 / 人均成本)标注「累计」,避免在周期视图被误读为当期。
- **移动端** —— 首页 / 热点雷达手机端间距与换行优化;市场热度手机端排版优化。

阻塞项 / 遗留:

- 当日另有 nginx 安全加固 + Stage B HTTPS 预置,见同日安全整改条目。

### 2026-07-01 — 黄金独立成页 + admin BI 改版 + 白名单登录 + 使用说明重做

交付内容:

- **黄金模块(独立 `/gold`)** —— 国内金 + 国际金 + 每日 AI 复盘;内外价差(汇率折算)喂给复盘;国内金取实时最新逐笔(而非开盘首笔)、涨跌对昨收算;K 线图 + 周期切换(分时 / 日 K / 周 K / 月 K);每日复盘加多轮追问(AI 对话);技术面参考(描述性)+ AI 复盘追问;复盘支持每日 / 每周 / 每月三周期;独立成页并入导航、从市场热度模块移出。
- **admin BI 改版** —— 概览结构化(按 流量 / 用户与转化 / 使用·分析 / 成本 分组);BI 风格改版(周期选择器 + 环比 Δ + 条形图);浅色 BI 主题(`.admin-light` 覆盖主题 token,启用完整品牌蓝阶梯);图表改品牌蓝单色渐进配色、KPI 数字统一中性;窗口访客改真去重(近 7 / 近 30,不再按日 uniques 相加);使用 / 分析改用全体用户口径(`run:` 事件,含匿名,不再只算登录报告);`runs` 表全体用户分析成本 / 标的精确统计;使用分析按登录 / 未登录拆分;用户活跃度拆分登录 / 未登录。
- **白名单登录** —— 登录用户免限速评 / Serenity,辩论可配置上限 + 后台限额设置;用户白名单(选定账号辩论也不限);登录支持邮箱 / 手机号 + 白名单可输入添加(含预授权)。
- **使用说明 + 首页** —— 使用说明更新(加黄金板块、修正登录口径)并加宽精简(两列布局,短标题 + 一句话);首页大标题字号调小(3xl/5xl → 2xl/4xl);四种分析模式改为醒目卡片(图标 + 名称 + 耗时说明);中文文案半角逗号统一改全角;`market-heat` 并行取数 + 启动预热 + stale-while-revalidate。
...
- **后端 `funds.py` + `GET /api/fund`** —— 基金概况(雪球 `fund_individual_basic_info_xq`:名称/类型/经理/规模/成立/基准/策略)、净值走势(`fund_open_fund_info_em`,开基与 ETF 通用,2000+ 点)、最新季度重仓股 Top10(`fund_portfolio_hold_em`,含占净值比例)、多周期收益(从净值算 1m/3m/6m/1y/YTD/成立来);ETF 名称/类型用全量代码表 `fund_name_em` 兜底(雪球对 ETF 不返回);6h 缓存。VPS 实测**全部接口可达**(连 ETF 实时 `fund_etf_spot_em` 都通,与个股的 push2 墙不同)。
- **前端 `/fund` 页** —— 代码查询(支持 `?code=` 深链)+ 概况卡 + 6 个周期业绩卡 + 净值曲线(降采样到 ~220 点)+ 重仓股条形图 + 基准/策略;左侧导航加「基金」;点查询埋 `run:fund`(后台可统计)。
- **验证** —— 公网实测:招商白酒 161725(净值 2696 点、Top10 茅台 18.33%、成立来 -48.4%)、沪深300ETF 510300(净值 3436 点、成立来 +401%、宁德 4.27%、名称兜底成功)。
- **基金搜索 + ETF实时 + AI点评(同日补全)**:
  - **搜索补全** `GET /api/fund-search` —— 查全量代码表(27108 只),按 代码/简称/拼音 打分匹配;前端 `/fund` 输入框自动补全。实测「全球科技」「易方达」「白酒」均命中。
  - **ETF 实时 + 折溢价** —— `fund_etf_spot_em`(60s 缓存,叠加在 6h 基金缓存上)取现价/IOPV/折溢价率/涨跌;并算**最大回撤**(成立来,从净值峰谷)。
  - **基金 AI 点评** `POST /api/fund-analyze`(SSE)—— `skill_runner.stream_fund_review`,基金分析师 prompt 覆盖 持仓集中度/行业押注/经理/规模/费率/回撤/相对基准/性价比;复用限流+预算,登录存报告 `mode=fund`。前端基金页「AI 点评」按钮流式渲染。实测 161725 点评准确(识别白酒前十大占 85.8%、单一行业押注、高波动)。
...
- **后台维度扩展 + 结构化(同日补)** —— 后台改为标签页(概览/流量/使用/邀请码/路径)。新增维度:分析点击按模式拆分(含 snapshot,点分析时埋 `run:<mode>`,不论登录)、分析次数 + LLM 累计花费、人均浏览、每日趋势图、时段分布(北京时间 UTC+8)、新用户每日注册、今日新/回访访客、最常分析的股票、按保存报告的模式拆分、用户活跃度(按 runs + 最近活跃)、邀请漏斗。
...
- **独立 `/overview` 页** —— 三块:今日热门行业、成交最活跃公司、本站今日分析最多;可点击直接跳分析。左侧导航加「热度」。
- **后端 `/api/market-overview`(缓存 5 分钟)**:
  - 热门行业 = 新浪行业板块 `stock_sector_spot("新浪行业")` 按涨幅 Top12 + 领涨股(列名 GBK 不稳,按位置取)。
  - 成交最活跃 = 新浪现货全表 `stock_zh_a_spot` 按成交额 Top20。
